...
namespace Neuron.Tests
{
    ...
    public class LogicGateTests
    {
        ...
        public void Train_WithActivationFunction_ShouldPredictLogicGateOutput(string truthTableString)
        {
            ...
            var neuron = new Neuron(weights, biasWeight, _stepActivation);
            IList<double[]> truthTable = ParseTable(truthTableString);

            for (var i = 0; i < 10; i++)    // train on the table 10 times through
            {
                PerformActionOnTruthTable(truthTable, (inputs, expected) =>
                {
                    var inputsList = inputs as IList<double> ?? inputs.ToList();
                    neuron.Train(inputsList, () => (expected - neuron.GetOutput(inputsList)));
                });
            }

            // assert the predition of the Neuron
            PerformActionOnTruthTable(truthTable, (inputs, expected) => Assert.AreEqual(expected, neuron.GetOutput(inputs)));
        }
